...
recommendations = hybrid_rec.recommend_batch(weights_array=weights, normalization_mode=Hybrid.MAX_MATRIX)
...
rec = CFUserBased()
rec.fit(urm_train=data.get_urm_train(), distance=ContentBasedRecommender.SIM_SPLUS, k=600, alpha=0.25, beta=0.5,
        shrink=10, l=0.5, c=0.25)
...
r_hat_array.append(sps.load_npz(base_path+'_slim_bpr_22-42-07.npz'))
...
#r_hat_array.append(sps.load_npz(base_path+'pureSVD_19-24-20.npz'))
#r_hat_array.append(sps.load_npz(base_path+'_CFuser_18-14-24.npz'))
...
hybrid_rec = Hybrid(r_hat_array, urm=data.get_urm_train())
