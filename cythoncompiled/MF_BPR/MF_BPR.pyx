...
cdef class MFBPR_Epoch:
    ...
    cdef double loss
    cdef double countwrong

    def __init__(self, URM, n_factors=10, learning_rate=1e-4, user_regularization=0.001, positive_item_regularization=0.001, 
    ...
    def sampleTriplet(self):
        ...
        user_seen_items = self.URM_indices[self.URM_indptr[user_id] : self.URM_indptr[user_id + 1]]
    ...
    def epochIteration(self):
        ...
        num_positive_interactions = int(len(self.URM_data) * 0.5)
    ...
    def update_factors(self, u, i, j):
        ...
        new_item_factor_j = self.item_factors[j, :] + self.learning_rate * (
            np.multiply(z, (np.negative(user_factor_c))) + np.multiply(self.negative_item_regularization, item_factors_c_j))
