...
class SLIM_BPR_USER(RecommenderBase):
    """
    ...
    def _fit_alreadyInitialized(self, epochs=30, logFile=None, URM_test=None, user_ids=None, filterTopPop=False,
                                minRatingsPerUser=1,
    ...
    def recommend_batch(self, userids, N=10, urm=None, filter_already_liked=True, with_scores=False,
        ...
        user_profile_batch = self.URM_train.T[userids]
        scores_array = self.W_sparse[userids].dot(self.URM_train.T).toarray()

        print('scores_array computed')

        if filter_already_liked:
            scores_array[user_profile_batch.nonzero()] = -np.inf

        print('scores_array filtered')
